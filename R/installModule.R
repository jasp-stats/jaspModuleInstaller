...
installJaspModuleImpl <- function(modulePkg, moduleLibrary, repos, onlyModPkg) {
  ...
  if (!file.exists(lockfilePath))
    generateBasicLockfile(lockfilePath, modulePkg, moduleLibrary)
...
generateBasicLockfile <- function(lockfilePath, modulePkg, moduleLibrary) {
  ...
  obj <- getModuleStatusObject(ifNotExists = "warning")
  ...
  updatedRecords <- renv::update(project = modulePkg, library = library)

  extractPackagesFromRecords <- function(records) {
    vapply(records[["Packages"]], `[[`, FUN.VALUE = character(1L), "name")
  }

  # basename(modulePkg) might also be sufficient, this needs more testing
  packagesToSnapshot <- union(basename(modulePkg), extractPackagesFromRecords(records))
  if (!isTRUE(updatedRecords))
    packagesToSnapshot <- union(packagesToSnapshot, extractPackagesFromRecords(updatedRecords))

  renv::snapshot(
    packages = updatedRecords,#basename(modulePkg),
    library  = library,
    exclude  = "jaspModuleInstaller",
